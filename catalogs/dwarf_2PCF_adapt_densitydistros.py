...
ax1.fill_between(theta, w+w_err, w-w_err, facecolor='#e0cda7',zorder=2,alpha=0.7)
ax1.axhline(y=0., linewidth=4, color='#191919',zorder=1, alpha=0.15)
#ax1.errorbar(dist, w, yerr=w_err, fmt='o', facecolor='#e0cda7')
ax1.set_xlabel(r'$\theta$ [arcmin]',fontsize=24)
ax1.set_ylabel(r'$w(\theta)$',fontsize=28)

textstr = '2-Point Correlation Function ($\pm$1$\sigma$)'
props = dict(boxstyle='round4,pad=0.3', facecolor='wheat', alpha=0.5)
ax1.annotate(textstr, xy=(0.45,0.8), xycoords=ax1.transAxes, fontsize=18, weight='bold', 
	va='center', ha='left', bbox=props, xytext=(0,0), textcoords='offset points', alpha=0.5)
#ax1.text('2-point correlation function',x=50,y=0.4)#numpoints=1,frameon=False,loc='upper right',prop={'size':20})
ax1.tick_params('both',length=6,width=2,which='major',labelsize=24)
...
#new_tick_locs = ax1.get_xticks()
print new_tick_locs
ax2.set_xticks(new_tick_locs)
ax2.set_xticklabels(tick_func_arcmin2kpc(new_tick_locs))
ax2.set_xlabel(r'$\theta$ [kpc]',fontsize=18)
ax2.tick_params('both',length=6,width=2,which='major',labelsize=16)
...
ax3.set_xlabel(r'$R_{gc}$ [arcmin]',fontsize=24)
ax3.set_ylabel(r'$\Sigma_N$ [arcmin$^{-2}$]',fontsize=24)
ax3.set_yscale('log')
ax3.legend((r'Total',r'Non-nucleated',r'Nucleated'),numpoints=1,frameon=False,loc='upper left',prop={'size':18})
ax3.tick_params('both',length=6,width=2,which='major',labelsize=24)
...
ax4.set_xlabel(r'$R_{gc}$ [kpc]',fontsize=18)
ax4.tick_params('both',length=6,width=2,which='major',labelsize=16)
ax4.tick_params('both',length=4,width=1,which='minor')

plt.tight_layout()

plt.savefig('dwarf_2PCF_adapt_densitydistros.pdf')
plt.show()