...
plt.legend((r'Total',r'Non-nucleated',r'Nucleated'),frameon=False,loc='upper left',prop={'size':12})

### ALL
normdat = len(ang_tot)*ang_bs
x_in = ang_tot[:, np.newaxis]
x_smpl = np.linspace(xmin, xmax, 1000)[:, np.newaxis]
#for kernel in ['gaussian', 'tophat', 'epanechnikov']:
kde = KernelDensity(kernel='epanechnikov', bandwidth=20.0).fit(x_in)
log_dens = kde.score_samples(x_smpl)
plt.plot(x_smpl[:, 0],np.exp(log_dens)*normdat,linestyle='-',color='#6b4423',linewidth=3,alpha=0.5)
#plt.scatter(ang_tot, 21.4 - 0.25 * np.random.random(ang_tot.shape[0]),marker='o',edgecolor='none',zorder='5',facecolor='#6b4423',alpha=0.4)

### NON-NUCLEATED
normdat = len(ang)*ang_bs
x_in = ang[:, np.newaxis]
x_smpl = np.linspace(xmin, xmax, 1000)[:, np.newaxis]
kde = KernelDensity(kernel='epanechnikov', bandwidth=20.0).fit(x_in)
log_dens = kde.score_samples(x_smpl)
plt.plot(x_smpl[:, 0],np.exp(log_dens)*normdat,linestyle='-',color='#bb1515',linewidth=2,alpha=0.5)
plt.scatter(ang, -0.4 - 0.25 * np.random.random(ang.shape[0]),marker='o',edgecolor='none',zorder='5',facecolor='#bb1515',alpha=0.4)

### NUCLEATED
normdat = len(ang_nuc)*ang_bs
x_in = ang_nuc[:, np.newaxis]
x_smpl = np.linspace(xmin, xmax, 1000)[:, np.newaxis]
kde = KernelDensity(kernel='epanechnikov', bandwidth=20.0).fit(x_in)
log_dens = kde.score_samples(x_smpl)
plt.plot(x_smpl[:, 0],np.exp(log_dens)*normdat,linestyle='-',color='#2a334f',linewidth=2,alpha=0.5)
plt.scatter(ang_nuc, -0.9 - 0.25 * np.random.random(ang_nuc.shape[0]),marker='o',edgecolor='none',zorder='5',facecolor='#2a334f',alpha=0.4)

#plt.legend((r'KDE(Total)'),loc='top right',frameon=False)

plt.tick_params('both',length=6,width=2,which='major',labelsize=12)
plt.tick_params('both',length=4,width=1,which='minor')
plt.xlabel(r'$\Phi$ [deg E of N]',fontsize=16)
plt.ylabel(r'Number',fontsize=16)
plt.tight_layout()

plt.savefig('dwarf_new_distros_adapt_v2.pdf')
plt.show()